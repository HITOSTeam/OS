...
#[cfg(target_arch = "riscv64")]
#[inline]
pub fn record_tlb_new_pte_refresh(svvptc_skip: bool) {
    if !DEBUG_PERF {
        return;
    }
    if svvptc_skip {
...
pub fn dump() -> String {
    ...
    #[cfg(target_arch = "riscv64")]
    let new_pte_tlb = {
        let tlb_new_pte_fences = TLB_NEW_PTE_FENCES.load(Ordering::Relaxed);
        let tlb_new_pte_svvptc_skips = TLB_NEW_PTE_SVVPTC_SKIPS.load(Ordering::Relaxed);
        alloc::format!(
            "tlb_new_pte_fences: {tlb_new_pte_fences}\n\
tlb_new_pte_svvptc_skips: {tlb_new_pte_svvptc_skips}\n"
        )
    };
